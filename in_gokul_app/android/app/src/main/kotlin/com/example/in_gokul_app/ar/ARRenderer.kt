...
class ARRenderer : GLSurfaceView.Renderer {
    companion object {
        private const val TAG = "INDOOR_AR_RENDERER"
    }
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        try {
            ...
            // 1. Render live camera background feed
            ...
            if (camera.trackingState == com.google.ar.core.TrackingState.TRACKING || camera.trackingState == com.google.ar.core.TrackingState.PAUSED) {
                ...
                // 2. Render active spatial anchors (3D Diamond Beacons)
                ...
                // 3. Render 3D board line ribbon connecting consecutive anchors
                if (anchorPositions.size >= 2) {
                    drawBroadPathRibbon(anchorPositions, floatArrayOf(0.0f, 1.0f, 0.5f, 0.9f))
                }

                // 4. Render 3D Active Navigation Route (Broad Ribbon Path + Directional Arrows + Destination Marker)
            ...
            Log.e(TAG, "FRAME_UPDATE_ERROR frame=$frameCount", e)
        } finally {
